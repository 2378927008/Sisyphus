...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await waitForState(
      window,
      (state) => state.activeHistoryId === "history-today-older",
      5000
    );
    ...
    await window.webContents.executeJavaScript(`
      (() => {
        ...
        document.querySelector('#editorBack').focus();
      })()
    `);
    ...
    const tabState = await readRendererState(window);
    if (!tabState.editorBackVisible || tabState.editorBackTabIndex < 0) {
      throw new Error("The narrow editor does not expose a reachable Back to history action.");
    }
